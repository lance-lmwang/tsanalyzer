# TsAnalyzer 运维与开发红线手册 (Maintenance Guardrails)
...
*   **红线**：任何对看板生成逻辑的修改，在提交前**必须**运行 `python3 tests/verify_scripts.py`。
*   **验证标准**：生成的 JSON 必须包含完整的 3 个 Plane 和 5 个 Tier。
...
*   **红线**：验证系统状态时，必须遵循 **AAT (Appliance Acceptance Test)** 数据流路径：
    1.  **探测引擎** (`curl :8081/metrics`)：确认是否有 raw 原始指标？
