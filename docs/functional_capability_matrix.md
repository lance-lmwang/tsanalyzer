...
| **ASI / DVB Hardware** | 💡 Concept | Direct baseband signal capture via PCIe cards (e.g., DekTec). |
...
| **Priority 3 (P3)** | 🔶 Partial | SDT and NIT version tracking/parsing. EIT not yet fully implemented. |
| **SCTE-35 Audit** | ✅ Implemented | Nanosecond-level alignment error between ad-triggers and Video I-Frames. |
| **PSI Version Tracking** | ✅ Implemented | Fast-path optimization to skip redundant parsing of unchanged tables. |
...
| **STC-to-UTC Mapping** | ✅ Implemented | Correlating stream time with physical wall-clock for log synchronization. |
...
| **Thumbnail Generation** | 📅 Planned | Sparse decoding of IDR frames to provide visual stream previews. |
| **Audio Loudness (LUFS)** | 💡 Concept | ITU-R BS.1770-4 compliance monitoring. |
| **BISS / SoftCam** | 💡 Concept | Real-time decryption of scrambled services. |
...
| **FFprobe Sidecar** | 📅 Planned | Embedding ffprobe/mediainfo metadata into the analysis reports. |
| **High-Performance TUI**| ✅ Implemented | `tsa_top` Ncurses interface for local console-based monitoring. |
...
| **Lua Pipeline Engine** | ⏸️ Paused | Scriptable graph-based stream orchestration (Astra integration). |
...
*   💡 **Concept**: Under research/consideration for long-term evolution.
