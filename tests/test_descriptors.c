...
void test_descriptor_registry() {
    ...
    tsa_descriptors_process(NULL, 100, ac3_desc, sizeof(ac3_desc), &stream_type);
    assert(stream_type == TSA_TYPE_AUDIO_AC3);

    stream_type = 0x06;
    uint8_t sub_desc[] = {0x59, 0x01, 0x00};
    tsa_descriptors_process(NULL, 101, sub_desc, sizeof(sub_desc), &stream_type);
    assert(stream_type == 0x59);

    stream_type = 0x06;
    uint8_t tele_desc[] = {0x56, 0x01, 0x00};
    tsa_descriptors_process(NULL, 102, tele_desc, sizeof(tele_desc), &stream_type);
...
static void custom_handler(struct tsa_handle *h, uint16_t pid, uint8_t tag, const uint8_t *data, uint8_t len,
                           uint8_t *stream_type) {
    (void)h;
    (void)pid;
    (void)tag;
...
void test_custom_handler() {
    ...
    tsa_descriptors_process(NULL, 200, custom_desc, sizeof(custom_desc), &stream_type);
