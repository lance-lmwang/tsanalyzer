...
void encode_pcr(uint8_t* pkt, uint64_t pcr_ticks) {
    ...
    pkt[4] = 7; pkt[5] = 0x10;
    pkt[6] = (uint8_t)(base >> 25); pkt[7] = (uint8_t)(base >> 17);
    pkt[8] = (uint8_t)(base >> 9); pkt[9] = (uint8_t)(base >> 1);
...
int main() {
    ...
    h->seen_pat = h->seen_pmt = true; h->last_pat_ns = h->last_pmt_ns = now;
    h->pid_eb_fill_double[0x0100] = 50000000; 
    h->stc_drift_slope = 1.0; 
    
    tsa_commit_snapshot(h, now);
    tsa_snapshot_full_t s1; tsa_take_snapshot_full(h, &s1);
    printf("   Health: %.1f, RST Net: %.1f, RST Enc: %.1f, MDI-DF: %.1f\n", 
           s1.summary.master_health, s1.predictive.rst_network_s, s1.predictive.rst_encoder_s, s1.stats.mdi_df_ms);
    ...
    printf("   Health: %.1f, RST Net: %.1f, MDI-DF: %.1f\n", s1.summary.master_health, s1.predictive.rst_network_s, s1.stats.mdi_df_ms);
