...
static void process_buffer_stateful(mock_stream_node_t *s, uint8_t *buf, int len, uint64_t now) {
    ...
    while (pos < len) {
        if (s->remainder_len > 0) {
            ...
            s->remainder_len += can_take; pos += can_take;
        ...
        if (buf[pos] == 0x47) {
            if (pos + 188 <= len) { tsa_process_packet(s->tsa, buf + pos, now); pos += 188; }
            else { s->remainder_len = len - pos; memcpy(s->remainder, buf + pos, s->remainder_len); pos = len; }
        } else { pos++; }
    }
}
...
void test_stateful_reassembly() {
    ...
    full_pkt[0] = 0x47; full_pkt[1] = 0x1F; full_pkt[2] = 0xFF;
