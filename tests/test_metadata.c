...
void test_metadata_parsing() {
    ...
    tsa_descriptors_process(&h, pmt_pid, service_desc, NULL);
    ...
    tsa_descriptors_process(&h, pmt_pid, lcn_desc, NULL);
    assert(p->lcn == 12);
    printf("  [PASS] LCN Descriptor parsed correctly.\n");

    // 4. Test parsing via Service ID (SDT style)
    uint16_t sid = 101;
    uint8_t service_desc2[] = {
        0x48, 15,
        0x01,
        4, 'A', 'B', 'C', 'D',
        8, 'S', 'E', 'R', 'V', 'I', 'C', 'E', '2'
    };
    tsa_descriptors_process(&h, sid, service_desc2, NULL);
