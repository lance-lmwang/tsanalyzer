...
void test_pat_version_rollover() {
    ...
    pat_v0[16] = (crc0 >> 24) & 0xFF; pat_v0[17] = (crc0 >> 16) & 0xFF;
    pat_v0[18] = (crc0 >> 8) & 0xFF; pat_v0[19] = crc0 & 0xFF;

    uint8_t pkt[188] = {0};
    pkt[0] = 0x47; pkt[1] = 0x40; pkt[2] = 0x00; pkt[3] = 0x10; 
    pkt[4] = 0x00; memcpy(pkt + 5, pat_v0, 20);
    ...
    uint8_t pat_v1[20] = {
        0x00, 0xB0, 0x11, 0x00, 0x01, 0xC3, 0x00, 0x00, // 0xC3 is version 1
        0x00, 0x03, 0xE3, 0x00, 0x00, 0x04, 0xE4, 0x00,
        0x00, 0x00, 0x00, 0x00 
    };
    uint32_t crc1 = mpegts_crc32(pat_v1, 16);
    pat_v1[16] = (crc1 >> 24) & 0xFF; pat_v1[17] = (crc1 >> 16) & 0xFF;
    pat_v1[18] = (crc1 >> 8) & 0xFF; pat_v1[19] = crc1 & 0xFF;

    pkt[3] = 0x11; // CC 1
    memcpy(pkt + 5, pat_v1, 20);
    tsa_decode_packet(h, pkt, 2000, &res);
    
    assert(h->pid_filters[0].last_version == 1);
    assert(h->program_count == 2);
    ...
    uint8_t pat_v31[20] = {
        0x00, 0xB0, 0x11, 0x00, 0x01, 0xFF, 0x00, 0x00, // 0xFF -> version 31 (11111)
        0x00, 0x05, 0xE5, 0x00, 0x00, 0x06, 0xE6, 0x00,
        0x00, 0x00, 0x00, 0x00 
    };
    uint32_t crc31 = mpegts_crc32(pat_v31, 16);
    pat_v31[16] = (crc31 >> 24) & 0xFF; pat_v31[17] = (crc31 >> 16) & 0xFF;
    pat_v31[18] = (crc31 >> 8) & 0xFF; pat_v31[19] = crc31 & 0xFF;

    pkt[3] = 0x12; // CC 2
    memcpy(pkt + 5, pat_v31, 20);
    tsa_decode_packet(h, pkt, 3000, &res);
    assert(h->pid_filters[0].last_version == 31);
