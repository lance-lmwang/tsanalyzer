...
- [ ] **Phase 3: Lua-Powered Deep Content Inspection (SCTE-35)**
  ...
  - [ ] Lua Script: Write `plugins/scte35_parser.lua` to decode `splice_info_section()`.
  - [ ] Integration: Bind the `0x86` PID detection in PMT to automatically route payloads to the Lua SCTE-35 plugin.
  - [ ] Validate: Feed a known SCTE-35 stream (or synthetic PCAP) and verify that the splice insert markers are accurately logged in JSON format.
