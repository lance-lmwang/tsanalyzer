...
## Background
Based on a comprehensive review of leading open-source broadcast analyzers (`libeasyice`, `ltntstools`, and `opensrthub`), `tsanalyzer` has achieved a high level of accuracy and performance. However, to scale toward a fully modular, enterprise-grade probe, several architectural and analytical optimizations are required. The current `tsa.c` is becoming a monolith, and advanced broadcast features like SCTE-35 and clock drift tracking are missing.

## Objectives
1. **Modular Architecture**: Decouple the monolithic `tsa.c` into domain-specific inspectors (inspired by `ltntstools`), improving maintainability and testability.
2. **Advanced Clock Analytics**: Implement System-Clock-to-PCR-Drift (ppm) tracking to identify encoder clock instabilities.
3. **Broadcast Event Detection**: Introduce SCTE-35 (Splice Information Table) parsing to detect ad-insertion points (inspired by `opensrthub`).
