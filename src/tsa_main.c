...
int main(int argc, char** argv) {
    ...
    while (g_keep_running && fread(pkt, 1, 188, f) == 188) {
        ...
        count++;
        now_ns += 188ULL * 8 * 1000000000 / 10000000;  // Simulated 10Mbps step
        ...
        if (count % 1000 == 0) {
            ...
            uint64_t p1_total =
                s->sync_loss.count + s->pat_error.count + s->cc_error.count + s->pmt_error.count + s->pid_error.count;
            uint64_t p2_total = s->transport_error.count + s->crc_error.count + s->pcr_repetition_error.count +
                                s->pcr_accuracy_error.count;
            ...
            for (uint32_t i = 0; i < snap.active_pid_count; i++) {
                ...
                if (p == 0x1FFF) {
                    stuffing_pct = (s->physical_bitrate_bps > 0)
                                       ? (double)s->pid_bitrate_bps[p] * 100.0 / (double)s->physical_bitrate_bps
                                       : 0;
                }
            }

            printf(
                "\r[PKTS: %lu] BITRATE: %.2f Mbps (Null: %.1f%%) | P1: %lu | VIDEO: %ux%u | V-BR: %.2f (%.2f-%.2f) "
                "Mbps | GOP: %u (%u-%u) | HEALTH: %.1f",
                count, (double)s->physical_bitrate_bps / 1e6, stuffing_pct, p1_total, w, h, (double)v_br / 1e6,
                (double)v_min / 1e6, (double)v_max / 1e6, gop, g_min, g_max, snap.predictive.master_health);
            fflush(stdout);
        }
    }
