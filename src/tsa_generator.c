...
static uint8_t h264_headers[] = {0x00, 0x00, 0x00, 0x01, 0x67, 0x42, 0x00, 0x1f, 0x95, 0xa8, 0x1e, 0x00,
                                 0x5b, 0x90, 0x00, 0x00, 0x00, 0x01, 0x68, 0xce, 0x3c, 0x80, 0x00, 0x00,
                                 0x00, 0x01, 0x06, 0x05, 0x08, 'G',  'A',  '9',  '4',  0x03, 0x00, 0x00,
                                 0x00, 0x00, 0x00, 0x01, 0x65, 0x00, 0x00, 0x00, 0x01};

/* Verified PAT (Program 1 -> PMT PID 0x1000) */
static uint8_t pat_pkt[] = {0x47, 0x40, 0x00, 0x10, 0x00, 0x00, 0xB0, 0x0D, 0x00, 0x01, 0xC1,
                            0x00, 0x00, 0x00, 0x01, 0xF0, 0x00, 0x2A, 0xB1, 0x04, 0xB2};

/* Verified PMT (Program 1 -> Video PID 0x0100) */
static uint8_t pmt_pkt[] = {0x47, 0x50, 0x00, 0x10, 0x00, 0x02, 0xB0, 0x17, 0x00, 0x01, 0xC1, 0x00, 0x00,
                            0xE1, 0x00, 0xF0, 0x00, 0x1B, 0xE1, 0x00, 0xF0, 0x00, 0x4E, 0x59, 0x3D, 0x1E};

static void write_pcr_pkt(uint8_t* p, uint8_t* cc, uint64_t pcr_27mhz) {
    memset(p, 0xFF, TS_PACKET_SIZE);
    p[0] = 0x47;
    p[1] = 0x40 | ((VIDEO_PID >> 8) & 0x1F);
    p[2] = VIDEO_PID & 0xFF;
    p[3] = 0x20 | ((*cc)++ & 0x0F);
    p[4] = 183;
    p[5] = 0x10;
    ...
    p[8] = (base >> 9) & 0xFF;
    p[9] = (base >> 1) & 0xFF;
    p[10] = ((base & 0x01) << 7) | 0x7E | ((ext >> 8) & 0x01);
    p[11] = ext & 0xFF;
}
...
static void write_video_pkt(uint8_t* p, uint8_t* cc, bool pusi, uint64_t pts_90k) {
    memset(p, 0xFF, TS_PACKET_SIZE);
    p[0] = 0x47;
    p[1] = (pusi ? 0x40 : 0x00) | ((VIDEO_PID >> 8) & 0x1F);
    p[2] = VIDEO_PID & 0xFF;
    p[3] = 0x10 | ((*cc)++ & 0x0F);
    if (pusi) {
        p[4] = 0x00;
        ...
        p[11] = 0x80;
        p[12] = 0x05;
        p[13] = 0x21 | (((pts_90k >> 30) & 0x07) << 1);
        p[14] = (pts_90k >> 22) & 0xFF;
        p[15] = ((pts_90k >> 15) & 0x7F) << 1 | 0x01;
        p[16] = (pts_90k >> 7) & 0xFF;
...
int main(int argc, char** argv) {
    ...
    uint8_t udp[UDP_PACKET_SIZE], cc_v = 0, cc_pat = 0, cc_pmt = 0;
    uint64_t pkts = 0;
    ...
    while (g_keep_running) {
        for (int i = 0; i < 7; i++) {
            ...
            if (pkts % 500 == 0) {
                memcpy(p, pat_pkt, sizeof(pat_pkt));
                memset(p + sizeof(pat_pkt), 0xFF, 188 - sizeof(pat_pkt));
                p[3] = (p[3] & 0xF0) | (cc_pat++ & 0x0F);
            } else if (pkts % 500 == 1) {
                memcpy(p, pmt_pkt, sizeof(pmt_pkt));
                memset(p + sizeof(pmt_pkt), 0xFF, 188 - sizeof(pmt_pkt));
                p[3] = (p[3] & 0xF0) | (cc_pmt++ & 0x0F);
            } else if (pkts % 40 == 0)
                write_pcr_pkt(p, &cc_v, pcr_27m);
            else
                write_video_pkt(p, &cc_v, (pkts % 1000 == 10), (stream_ns * 9) / 100000);
            pkts++;
        }
