        ...
        if (mg_match(hm->uri, mg_str("/metrics"), NULL)) {
            ...
            for (int i=0; i<MAX_S; i++) {
                off += snprintf(buf+off, sizeof(buf)-off, 
                    ...
                    g_s[i].id, (unsigned long long)g_s[i].pkts,
                    g_s[i].id, (unsigned long long)g_s[i].br);
            }
...
int main() {
    ...
    for (int i=0; i<MAX_S; i++) {
        ...
        sa.sin_family = AF_INET; sa.sin_port = htons(19001 + i);
        int r = 1; setsockopt(g_s[i].fd, SOL_SOCKET, SO_REUSEADDR, &r, sizeof(r));
