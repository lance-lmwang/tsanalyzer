...
static void print_stats(tsp_handle_t* h, uint32_t ts_per_udp) {
    ...
    tsa_info("STATS", "Pkts: %lu, Source: %.2f Mbps, Output: %.2f Mbps (Target: %.2f Mbps)",
             total_ts, (double)est_rate / 1000000.0, cur_mbps, (double)det_rate / 1000000.0);
}
...
int main(int argc, char* argv[]) {
    ...
    while ((opt = getopt_long(argc, argv, "b:i:p:S:PCm:lf:t:c:h", long_options, NULL)) != -1) {
        switch (opt) {
            case 'b': cfg.bitrate = strtoull(optarg, NULL, 10); break;
            case 'i': cfg.dest_ip = optarg; break;
            case 'p': cfg.port = (uint16_t)atoi(optarg); break;
            case 'S': cfg.url = optarg; break;
            case 'P': cfg.mode = TSPACER_MODE_PCR; break;
            case 'C': cfg.mode = TSPACER_MODE_CBR; break;
            case 'm': cfg.pcr_pid = (uint16_t)strtol(optarg, NULL, 0); break;
            case 'l': loop_file = 1; break;
            case 'f': input_file = optarg; break;
            case 't': cfg.ts_per_udp = (uint32_t)atoi(optarg); break;
            case 'c': cfg.cpu_core = atoi(optarg); break;
