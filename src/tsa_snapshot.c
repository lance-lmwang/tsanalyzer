...
static void tsa_calc_stream_bitrate(tsa_handle_t* h, uint64_t stc_dt) {
    ...
    h->live->mdi_mlr_pkts_s = (double)((h->live->cc_loss_count - h->prev_snap_base->cc_loss_count) * NS_PER_SEC) / stc_dt;
...
void tsa_commit_snapshot(tsa_handle_t* h, uint64_t n) {
    ...
    float rn = 10.0f - (float)(jitter_ms / 5.0f); if (rn < 0) rn = 0;
    float re = 100.0f - (float)(drift_abs * 2.0f); if (re < 0) re = 0;
    double cn = jitter_ms / 50.0; if (cn > 1.0) cn = 1.0;
    double ce = drift_abs / 100.0; if (ce > 1.0) ce = 1.0;
    ...
    for (uint32_t i = 0; i < ptc; i++) {
        ...
        sn->pids[ai].width = h->pid_width[p]; sn->pids[ai].height = h->pid_height[p];
        sn->pids[ai].profile = h->pid_profile[p]; sn->pids[ai].level = h->pid_level[p];
        sn->pids[ai].chroma_format = h->pid_chroma_format[p]; sn->pids[ai].bit_depth = h->pid_bit_depth[p];
        sn->pids[ai].exact_fps = h->pid_exact_fps[p]; sn->pids[ai].audio_sample_rate = h->pid_audio_sample_rate[p];
        sn->pids[ai].audio_channels = h->pid_audio_channels[p]; sn->pids[ai].gop_n = h->pid_last_gop_n[p];
        sn->pids[ai].gop_min = h->pid_gop_min[p]; sn->pids[ai].gop_max = h->pid_gop_max[p];
        sn->pids[ai].gop_ms = h->pid_gop_ms[p]; sn->pids[ai].i_frames = h->pid_i_frames[p];
        sn->pids[ai].p_frames = h->pid_p_frames[p]; sn->pids[ai].b_frames = h->pid_b_frames[p];
    ...
    sn->active_pid_count = ai; sn->stats = *h->live;
...
int tsa_take_snapshot_full(tsa_handle_t* h, tsa_snapshot_full_t* s) {
    if (!h || !s) return -1;
    uint8_t a = atomic_load_explicit(&h->double_buffer.active_idx, memory_order_acquire);
    *s = *h->double_buffer.buffers[a]; return 0;
}

int tsa_take_snapshot_lite(tsa_handle_t* h, tsa_snapshot_lite_t* s) {
    if (!h || !s) return -1;
    s->total_packets = h->live->total_ts_packets; s->physical_bitrate_bps = h->live->physical_bitrate_bps;
    s->active_pid_count = h->pid_tracker_count; s->signal_lock = h->signal_lock;
    uint8_t a = atomic_load_explicit(&h->double_buffer.active_idx, memory_order_acquire);
    s->master_health = h->double_buffer.buffers[a]->summary.master_health; return 0;
}
