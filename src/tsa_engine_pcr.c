...
static void pcr_on_ts(void* self, const uint8_t* pkt) {
    ...
    if ((pkt[3] & 0x20) && pkt[4] > 0 && (pkt[5] & 0x10)) {
        ...
        if (h->last_pcr_ticks > 0 && h->last_pcr_interval_bitrate_bps > 0) {
            ...
            if (diff < -((int64_t)1 << 41)) diff += ((int64_t)1 << 42);
            else if (diff > ((int64_t)1 << 41)) diff -= ((int64_t)1 << 42);
        ...
        if (++ctx->pcr_count_since_regress >= 16) {
            ...
            if (ts_pcr_window_regress(&h->pcr_window, &sl, &ic, &pa) == 0) {
                ...
                double instant_drift = (sl - 1.0) * 1000000.0; if (instant_drift > 1000000.0) instant_drift = 1000000.0; else if (instant_drift < -1000000.0) instant_drift = -1000000.0;
        ...
        if (h->last_pcr_ticks > 0) {
            ...
            if (dt_ticks < ((uint64_t)1 << 41)) {
                ...
                if (h->live->pcr_bitrate_bps == 0) h->live->pcr_bitrate_bps = interval_br;
                else h->live->pcr_bitrate_bps = (interval_br * 2 + h->live->pcr_bitrate_bps * 8) / 10;
