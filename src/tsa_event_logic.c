void tsa_push_event(tsa_handle_t* h, tsa_event_type_t type, uint16_t pid, uint64_t val) {
    ...
    switch (type) {
        case TSA_EVENT_SYNC_LOSS:
            tsa_update_alert_state(h, TSA_ALERT_SYNC, true, "SYNC", pid);
            break;
        case TSA_EVENT_PAT_TIMEOUT:
            tsa_update_alert_state(h, TSA_ALERT_PAT, true, "PAT", pid);
            break;
        case TSA_EVENT_PMT_TIMEOUT:
            tsa_update_alert_state(h, TSA_ALERT_PMT, true, "PMT", pid);
            break;
        case TSA_EVENT_CC_ERROR:
            tsa_update_alert_state(h, TSA_ALERT_CC, true, "CC", pid);
            break;
        case TSA_EVENT_CRC_ERROR:
            tsa_update_alert_state(h, TSA_ALERT_CRC, true, "CRC", pid);
            break;
        case TSA_EVENT_PCR_REPETITION:
        case TSA_EVENT_PCR_JITTER:
            tsa_update_alert_state(h, TSA_ALERT_PCR, true, "PCR", pid);
            break;
        default:
            break;
    }
