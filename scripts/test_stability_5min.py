...
def run_test_session():
    ...
    server_proc = subprocess.Popen(["./build/tsa_server", "http://0.0.0.0:8080"], stdout=log_f, stderr=log_f)
    time.sleep(2)

    # 3. Create Stream Task
    try:
        print(f"Creating Stream Task: {STREAM_ID}")
        requests.post(f"http://localhost:8080/streams?id={STREAM_ID}", timeout=5)
