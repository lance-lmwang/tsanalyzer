...
for i in {0..6}; do fuser -k -9 $((8088 + i))/udp || true; done
...
for i in {0..6}; do
    taskset -c $((i+1)) ./build/tsp -P -l -t 7 -i 127.0.0.1 -p $((8088 + i)) -f "$SAMPLE" > /dev/null 2>&1 &
done
...
while true; do
    ...
    if [ "$CUR_CC" -gt "$BASE_CC" ]; then
        echo "[FAIL] FAILURE: CC Leakage detected! (+$((CUR_CC - BASE_CC)))"
...
echo "[PASS] FINAL VERDICT: 5-MIN STABILITY PASSED (CC INCREMENT: 0)"
