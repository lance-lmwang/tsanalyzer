...
run_test() {
    ...
    # Transcoding (Industrial V2 settings)
    $FFMPEG -y -hide_banner -thread_queue_size 128 -rw_timeout 30000000 -fflags +discardcorrupt \
        ...
        -pat_period 0.2 -sdt_period 0.25 -mpegts_start_pid 0x21 -mpegts_tstd_mode 1 -mpegts_tstd_debug 1 -mpegts_tstd_window 30 \
...
GEOF
        ...
        TARGET_DELAY=$(awk "BEGIN {print $MUXDELAY * 1200}")

        tail -n +6 "$DATA_FILE.raw" | \
        awk -v target_delay="$TARGET_DELAY" '{
            ...
            if(count>0) {
                ...
                printf "Delay Stats   : Max Video Delay: %dms (Target < %dms)\n", max_v, target_delay;
            } else {
...
case "$1" in
    ...
    1080p_high) run_test "1080p_high" "af2_srt_src.ts" "5500k" "6900k" 35 2750 ;;
    all)
        $0 sd
        $0 720p
        $0 1080i
