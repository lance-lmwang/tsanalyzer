...
bitrate_kb=1600
muxrate=2000000
dst="${OUT_DIR}/tstd_stable.ts"
log_file="${OUT_DIR}/tstd_stable.log"
test_duration=30 # 5 minutes soak test

echo "[*] Starting T-STD Stable Soak Test (Duration: ${test_duration}s)..."
echo "[*] Output directory: $OUT_DIR"
echo "[*] Log file: $log_file"

# --- FFmpeg Command Logic ---
# Optimized with libwz264 CBR settings and T-STD V3 Pacer
cmd="$ffm -y -v debug -stream_loop -1 -autorotate 0 -rw_timeout 10000000 -i '$src' \
      -t $test_duration \
      -map 0:v:0 -map 0:a:0 \
