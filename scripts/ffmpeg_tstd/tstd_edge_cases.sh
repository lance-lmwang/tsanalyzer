...
grep -q "Initializing Engine" "$OUT_DIR/startup.log"
...
V_DUR=$(mediainfo --Inform="Video;%Duration%" "$OUT_DIR/drain.ts" | grep -E "^[0-9]+$" || echo 0)
A_DUR=$(mediainfo --Inform="Audio;%Duration%" "$OUT_DIR/drain.ts" | grep -E "^[0-9]+$" || echo 0)
DIFF=$(( A_DUR - V_DUR ))
if [ "$V_DUR" -gt 0 ] && [ "$DIFF" -le 2500 ] && [ "$DIFF" -ge 0 ]; then
    echo -e "\033[32m[PASS]\033[0m Aligned: V=${V_DUR}ms, A=${A_DUR}ms (Offset: ${DIFF}ms)."
else
    echo -e "\033[31m[FAIL]\033[0m Alignment fail: V=${V_DUR}ms, A=${A_DUR}ms (Offset: ${DIFF}ms)."
    GLOBAL_SUCCESS=0
fi
