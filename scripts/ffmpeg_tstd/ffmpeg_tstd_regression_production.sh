...
run_benchmark() {
    ...
    # Analyze Duration
    local actual=$($ffp -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 "$dst")
    echo "    - Duration: ${actual}s"

    # Analyze Bitrate (PID 0x21)
    echo "    - Bitrate Audit:"
    python3 "$analyzer" "$dst" --pid 0x0021 --pcr 0x0021 --skip 5.0 | grep -E "Mean Bitrate|Fluctuation" | sed 's/^/      /'
}
