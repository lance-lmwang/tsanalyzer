#!/bin/bash
# T-STD V3 PCR-Based PROMAX Alignment Matrix (Production Params)
# Target: 64k fluctuation goal with 1s VBV

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT_DIR="$(cd "$SCRIPT_DIR/../.." && pwd)"
OUT_DIR="${ROOT_DIR}/output/final_comparison"
mkdir -p "$OUT_DIR"
...
echo "  Parameters: GOP 1s, VBV 1s, 600kbps, AI Pre-process"
echo "=============================================================================="
...
run_mode() {
    local mode=$1
    local name=$2
    local dst="${OUT_DIR}/m${mode}_final.ts"

    # Run encode with production parameters (no -t, approx 3 mins)
    # Using 180s duration to match your 3-minute request
    $ffm -hide_banner -y -thread_queue_size 128 -rw_timeout 30000000 -fflags +discardcorrupt \
        ...
        -mpegts_start_pid 0x21 -mpegts_tstd_mode $mode -max_muxing_queue_size 4096 -max_interleave_delta 30000000 \
        -f mpegts "$dst" > /dev/null 2>&1

    # Audit using PCR Time axis (1504ms window)
