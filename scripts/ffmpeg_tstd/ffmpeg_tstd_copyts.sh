...
bitrate_kb=1600
muxrate=2000000
dst="${OUT_DIR}/tstd_copyts.ts"
log_file="${OUT_DIR}/tstd_copyts.log"
test_duration=60

echo "[*] Starting T-STD copyts Test..."
echo "[*] Output directory: $OUT_DIR"
echo "[*] Log file: $log_file"

# --- FFmpeg Command Logic ---
# Optimized with libwz264 CBR settings and T-STD V3 Pacer
cmd="$ffm -y -copyts -v trace -i '$src' \
      -t $test_duration \
      -map 0:v:0 -map 0:a:0 \
      -c:a libfdk_aac -profile:a aac_low -b:a 128k \
      -vf yadif=mode=0:parity=auto:deint=1,scale=1920:1080,fps=fps=25 \
      -vcodec libwz264 \
      -wz264-params bframes=0:keyint=25:vbv-maxrate=$bitrate_kb:vbv-bufsize=$bitrate_kb:nal-hrd=cbr:force-cfr=1:aud=1 -threads 4 \
...
echo "[4/5] Running Industrial-grade CBR Audit..."
"${ROOT_DIR}/scripts/ffmpeg_tstd/tstd_cbr_audit.sh" "$dst"

echo "[4/5] Normalizing results (skipping 2s startup silence using raw dd)..."
# Skip 2660 TS packets (approx 2s at 2Mbps) to get steady state metrology
dd if="$dst" of="${dst}.stable.ts" bs=188 skip=2660 2>/dev/null

echo "[5/5] Running Industrial-grade TSDuck PCR Audit..."
#tsp -I file "${dst}.stable.ts" -P analyze -P pcrverify -P continuity -O drop
"${ROOT_DIR}/build/tsa_cli" "${dst}.stable.ts"

echo ""
echo "[*] copyts Test finished."
