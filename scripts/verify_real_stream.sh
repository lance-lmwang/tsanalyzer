...
# Fetch Live Metrics
LIVE_BR=$(curl -s http://localhost:12345/metrics | grep "tsa_physical_bitrate_bps" | awk '{print $2}')
LIVE_HEALTH=$(curl -s http://localhost:12345/metrics | grep "tsa_health_score" | awk '{print $2}')
